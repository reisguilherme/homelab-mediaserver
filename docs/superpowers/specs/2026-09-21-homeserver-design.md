...
| D06 | Para filmes, escolher legenda na ordem pt-BR da mesma release, pt-BR de outra release com duração compatível, inglês da mesma release, inglês de outra release com duração compatível; dispensar legenda se a faixa original for explicitamente pt-BR |
...
3. Dentro de cada classe, preferir 2160p a 1080p e então Dolby Vision e Dolby Atmos quando identificáveis. Entre versões equivalentes, preferir mais seeds informados pelo indexador antes de desempatar por tamanho. O tamanho real do torrent precisa caber no espaço disponível após os bytes pendentes da fila.
...
A ordem acima substitui a antiga precedência de dual áudio sobre classe de qualidade. Na aquisição atual, a fonte é classificada pelos campos de qualidade do Radarr e Dolby Vision/Atmos são preferências derivadas do nome da release, não garantias de codec. Para filmes, o download pode começar após a validação do manifesto e da capacidade sem consulta prévia ao SubDL; nesse caso, o arquivo permanece em `/data/torrents`, fora da biblioteca do Jellyfin, até que o finalizador confirme uma legenda elegível ou uma faixa original explicitamente pt-BR. Séries sem sidecar mantêm a obtenção e persistência do SRT antes do permit.

Para filmes, a busca externa usa o ID TMDb verificado do filme e classifica os SRTs válidos nesta ordem: (0) pt-BR da mesma release; (1) pt-BR de outra release com duração compatível; (2) inglês da mesma release; (3) inglês de outra release com duração compatível. O SubDL V1 identifica inglês como `EN`, sem garantir o dialeto en-US. A release exata tem prioridade, mas um nome diferente não exclui a legenda. Nesse caso, os marcadores de edição ou corte, como `Extended` e `Director's Cut`, devem corresponder aos do vídeo. A duração é avaliada pela cobertura temporal dos cues do SRT em relação à duração medida do vídeo, com margem para créditos finais; isso não garante sincronismo perfeito.

Para séries, a busca externa mantém a correspondência exata da release e, subsidiariamente, a legenda do mesmo título, episódio e família de fonte. Esse fallback pode exigir ajuste manual de sincronização quando os cortes diferirem. Na finalização, uma faixa inglesa embutida também serve como fallback quando o `ffprobe` identifica explicitamente o idioma, um codec de texto reproduzível e uma faixa não marcada como `forced`; a busca pt-BR continua tendo prioridade. Idioma `por` genérico e legendas embutidas sem esses metadados não são aceitos automaticamente. A ausência de metadados verificáveis ou de espaço continua deixando o pedido pendente.
...
Na fila do qBittorrent, filmes admitidos e incompletos podem ultrapassar filmes com menos seeds. Usar a contagem observada de seeds no swarm quando disponível e a contagem informada pelo indexador enquanto o torrent ainda aguarda anúncio. Episódios e temporadas mantêm a sequência cronológica e precedência sobre os filmes: somente o primeiro episódio faltante de cada série pode avançar, independentemente dos seeds de episódios posteriores. Reordenar somente depois de reconciliar todas as temporadas aprovadas no Sonarr; se alguma não puder ser conferida, manter a fila como está nesse ciclo. A prioridade não autoriza um novo torrent, nem inicia um torrent parado. Contagens de seeds indicam disponibilidade, não velocidade garantida.
