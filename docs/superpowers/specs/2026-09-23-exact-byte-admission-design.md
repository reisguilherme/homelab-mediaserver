...
1. O worker registra todos os pedidos aprovados com compromisso inicial de zero byte antes de começar as buscas de fontes, que podem ser demoradas. Isso permite buscar sem ocupar disco e evita que um pedido antigo em `waiting_space` continue assim apenas por estar depois de outra busca. Pedidos sem fonte elegível continuam pesquisáveis e não impedem os demais.
